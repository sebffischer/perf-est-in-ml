...
1. Use `future` (or soon also `mirai`) to parallelize the execution of a resampling or a benchmark.
1. Soon it will be possible to use `mirai` as a plug-in replacement for `future`, which is significantly faster.
...
Note that you want to pay attention to any NEWS about the `mirai` integration to `mlr3`, which will really simplify some configuration options and make everything faster.